...
float scaleTemperatureToC(float t)
{
  float _t;
  switch(my_tempscale){
    case TEMP_CELSIUS:
...
void flash()
{
  ...
  if (testAccel())
    getAccSample();
  Tilt = calculateTilt();
  Temperatur = scaleTemperatureToC(calculateTemp(scaleTemperature(getTemperature(false)), my_tempadjust));
...
void setup()
{
  ...
  Temperatur = scaleTemperatureToC(calculateTemp(scaleTemperature(getTemperature(true)), my_tempadjust));
  CONSOLE("Temp: ");
