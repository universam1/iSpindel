...
void postConfig()
{
  ...
  if (myData.my_hassio)
  {
    sender.enableHassioDiscovery(myData.my_server, myData.my_port, myData.my_username, myData.my_password, myData.my_name, tempScaleLabel(myData.my_tempscale));
  }
...
#if API_MQTT_HASSIO
  {
    auto hassio = myData.my_api == DTMQTT && String(custom_hassio.getValue()) == "checked";
    my_hassio_changed = myData.my_hassio != hassio;
    myData.my_hassio = hassio;
  }
#endif
...
bool processResponse(String response)
{
  ...
  if (!error && doc.containsKey("interval"))
  {
    ...
    if (interval != myData.my_sleeptime && interval < 24 * 60 * 60 && interval > 10)
    {
...
bool uploadData(uint8_t service)
{
      ...
      (service == DTHTTPS))
  {
    ...
    {
      CONSOLELN(F("\ncalling HTTPS"));
      return sender.sendHTTPSPost(myData.my_server, myData.my_uri);
    }
  }
  ...
  if (service == DTBRICKS)
  {
    ...
    if (myData.my_token[0] != 0)
    {
      ...
      sender.add("apikey", myData.my_token); // use the token field as vessel for the api key
    }
    else
    {
    ...
    {
      myData.my_sleeptime = my_sleeptime_candidate_s;
      return true;
    }
    else
    {
...
void setup()
{
  ...
  if (shouldStartConfig(validConf))
  {
    ...
    {
      // test if ssid exists
      if (WiFi.SSID() == "" && myData.my_ssid != "" && myData.my_psk != "")
      {
